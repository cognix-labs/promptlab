...
async def main():
    ...
    openrouter_model = OpenRouter(model_config=inference_model_config)

    # Create embedding model config
    # Get embedding model name from environment variable or use the same as inference model
    embedding_model_name = os.environ.get("OPENROUTER_EMBEDDING_MODEL", model_name)
    ...
    embedding_model = OpenRouter_Embedding(model_config=embedding_model_config)
...
if __name__ == "__main__":
    ...
    if args.embedding_model:
        os.environ["OPENROUTER_EMBEDDING_MODEL"] = args.embedding_model
    if args.port:
