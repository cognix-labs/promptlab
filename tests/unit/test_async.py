...
class TestAsyncSupport(unittest.TestCase):
    ...
    def test_azure_openai_async(self, mock_async_client):
        ...
        # Create a mock for the async client that returns a pre-set result
        async def mock_create(*args, **kwargs):
            return mock_completion

        mock_async_client_instance = MagicMock()
        mock_async_client_instance.chat.completions.create = mock_create
        mock_async_client.return_value = mock_async_client_instance

        # Create an instance of AzOpenAI
        model = AzOpenAI(model_config)

        # Test async invocation using a new event loop
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)

        try:
            result = loop.run_until_complete(model.ainvoke("System prompt", "User prompt"))
        finally:
            loop.close()
